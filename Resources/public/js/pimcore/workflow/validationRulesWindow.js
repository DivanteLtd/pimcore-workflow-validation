pimcore.registerNS("pimcore.plugin.workflowgui.validationRulesWindow");

pimcore.plugin.workflowgui.validationRulesWindow = Class.create({
    ...
    getChecked: function (classId) {
        ...
        for (var i = 0; i < validation.classes.length; i++) {
            if (validation.classes[i].id == classId) {
                return validation.classes[i].rules;
            }
        }
        return [];
    },
    ...
    save: function (classId, checked) {
        ...
        for (var i = 0; i < validation.classes.length; i++) {
            if (validation.classes[i].id == classId) {
                validation.classes[i].rules = rules;
